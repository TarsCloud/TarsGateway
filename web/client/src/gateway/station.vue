<template>
  <div class="page_operation_templates">
    ...
    <let-table ref="table" :data="items" :empty-msg="$t('common.nodata')">
      <let-table-column
        :title="$t('gateway.stationId')"
        prop="f_station_id"
        width="20%"
      ></let-table-column>
      <let-table-column
        :title="$t('gateway.stationName')"
        prop="f_name_cn"
        width="20%"
      ></let-table-column>
      <let-table-column
        :title="$t('gateway.monitorUrl')"
        prop="f_monitor_url"
        width="25%"
      ></let-table-column>
      <let-table-column
    ...
    <let-modal
      ...
      :title="
        (viewModal.model ? viewModal.model.f_name_cn : '') +
          ' ' +
    ...
    >
      ...
      <let-tabs v-if="stationMode == 'http'">
        <let-tab-pane :tab="$t('gateway.router.title')">
          <HttpRouter
            v-if="viewModal.model"
            :station="viewModal.model"
          ></HttpRouter>
        </let-tab-pane>
...
export default {
  ...
  methods: {
    ...
    fetchData(curr_page) {
      ...
      return this.$ajax
        .getJSON("/api/station_list", this.query)
        .then((data) => {
          loading.hide();
    ...
    saveItem() {
      if (this.$refs.detailForm.validate()) {
        ...
        const url = model.f_id ? "/api/update_station" : "/api/add_station";
    ...
    removeItem(d) {
      this.$confirm(
        ...
        .then(() => {
          const loading = this.$Loading.show();
          this.$ajax
            .postJSON("/api/delete_station", { f_id: d.f_id })
            .then(() => {
              loading.hide();
              this.fetchData().then(() => {
                this.$tip.success(this.$t("common.success"));
