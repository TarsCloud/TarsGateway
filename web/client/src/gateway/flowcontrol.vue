<template>
  <div>
    <let-form
      ref="detailForm"
      itemWidth="200px"
      @submit.native.prevent="saveItem"
    >
      ...
      <let-form-item>
        <let-button
          ...
          type="button"
          theme="primary"
          >{{ $t("operate.modify") }}</let-button
        >
        <let-button v-if="editing" size="small" type="submit" theme="primary">{{
          $t("operate.save")
        }}</let-button>
      </let-form-item>
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      return this.$ajax
        .getJSON("/api/get_flowcontrol", {
          f_station_id: this.station.f_station_id,
        })
        .then((data) => {
          //loading.hide();
          this.form = data.f_duration
            ? data
            : { f_duration: 60, f_max_flow: 0 };
        })
        .catch((err) => {
          //loading.hide();
          this.$tip.error(
    ...
    saveItem() {
      if (this.$refs.detailForm.validate()) {
        ...
        const url = "/api/upsert_flowcontrol";
        const loading = this.$Loading.show();
        this.$ajax
          .postJSON(url, model)
          .then(() => {
